...
struct VideoHostView: UIViewRepresentable {
    ...
    func makeUIView(context: Context) -> LiveKit.VideoView {
        ...
        v.mirrorMode = mirror ? .mirror : .auto
        v.track = track
...
struct LiveRoomView: View {
    ...
    // 撮影者が被写体からガイドを受信したかどうか
    @State private var hasReceivedGuideFromSubject = false
    ...
    // 撮影者がお手本を出すかどうか
    private var shouldShowReference: Bool {
        if role == .photographer {
            return hasReceivedGuideFromSubject && (referenceImage != nil)
        } else {
    ...
    private var referenceOverlay: some View {
        if shouldShowReference, let ref = referenceImage {
    ...
    private func handleData(_ data: Data, topic: String) {
        ...
        do {
            ...
            if role == .photographer {
                ...
                // 被写体からガイドを受信したので、お手本表示を有効化
                hasReceivedGuideFromSubject = true
            }
